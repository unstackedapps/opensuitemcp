...
- **Upgrading: a client that re-authorizes needs an agent waiting for it.** Agents connected before this release keep working — the migration marks them connected — but the consent screen no longer creates anything, so re-adding a connector without first creating an agent set to Sign-in lands on "Nothing is waiting to connect", with a link to the portal. This is the intended behaviour, not a failure, and it is the one thing worth knowing before upgrading
...
- **Agent access tells you how to connect, per client.** They all speak the same protocol and every one of them asks for it differently: a URL in a dialog, a CLI flag, `mcpServers`, `servers`, `url`, `httpUrl`. The differences are not interesting but they are load-bearing — a server pasted under Gemini CLI's `url` means SSE and simply never connects — so [Connect an agent](https://opensuitemcp.com/docs/connect-an-agent) hands over the exact thing to paste, per client, rendered from the same file the app validates against. **App Portal → Agent access** says whether sign-in can work on this install at all before anyone spends twenty minutes on it: it needs `AUTH_URL` set and HTTPS, and an agent key needs neither
...
- **A refresh token could be used as an access token, and the other way round.** Both kinds live in one table and share one id namespace, and the code decided which kind it was holding by looking at the prefix the caller sent rather than at the stored row. Re-labelling a refresh token `osmcp_at_` therefore produced a working MCP credential with the refresh token's sixty-day lifetime — one that survived its own rotation, since rotation marks a token consumed and the access path never looked at that, and one reuse detection could never see, because it was never presented to the token endpoint. Every lookup is now constrained to the kind stored on the row, and revocation branches on what the row *is* rather than on what the caller called it. Found by adversarial review before release; no install had a client that could have triggered it accidentally, and `scripts/oauth-security-regression.ts` now fails if it comes back
- **Discovery documents are no longer cached when their contents can be steered.** Each one embeds this install's origin as `issuer` and `token_endpoint`, and without `AUTH_URL` that origin is derived from `X-Forwarded-Host` — while the response said `Cache-Control: public`. A shared cache in front of the app could be handed a document built from someone else's header and serve it to real clients, who would then post their authorization code to whatever endpoint it named. Public caching now requires a pinned origin, and `Vary` is sent either way
- **The client metadata document fetch cannot be pointed at your own network.** `/api/oauth/token` takes a `client_id` from an unauthenticated caller and fetches it, which made it a request forwarder into anything the app could reach — the database host, the Docker network, a cloud metadata service on `169.254.169.254`. Every address a name resolves to must now be publicly routable, the body is capped at 64 KB, and the work is counted against a shared budget as well as the per-client one, which an attacker escapes simply by varying the id they send
- **A replayed authorization code no longer revokes anything until the caller proves possession of the PKCE verifier.** Revoking first meant a leaked code plus a public `client_id` was enough to kill a victim's live tokens without proving anything
- **A reused refresh token no longer strands the agent.** Reuse revokes every token on the authorization, which is correct, and the client is told to sign in again — but the consent screen only offers agents with no client bound, so there was nothing there to sign in to, while the portal still showed the agent as active. The only way out was deleting it and rebuilding its name, persona and pinned account. The authorization now returns to *Awaiting connection*, so signing in again reconnects the same agent. This was reachable by an ordinary retried request, not just by an attacker

### 📝 Changed

- **The connection how-to is written once.** It lived in six places — a TypeScript module feeding the in-app guide, two hand-written docs pages, the README, the protocol reference, and the hosted reference page — and had already drifted: the public page carried a single generic `headers` example that was right for Cursor and wrong for VS Code and Gemini CLI, so a reader copying it under the Gemini section got `url` where `httpUrl` is required, which is the silent failure that same page warned about four lines earlier. Three clients had lost their agent-key instructions entirely. Now `lib/mcp/connect-clients.ts` is the only copy: the in-app guide and the public page render from it, `docs/connect-an-agent.md` is generated from it by `scripts/generate-connect-doc.ts --check`, and the README and protocol reference link rather than repeat
