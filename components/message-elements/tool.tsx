...
export const Tool = ({ className, ...props }: ToolProps) => (
  <Collapsible
    className={cn(
      "not-prose mb-4 w-full min-w-0 max-w-full rounded-md border",
      className,
    )}
    {...props}
  />
...
const getStatusBadge = (status: ToolUIPart["state"]) => (
  ...
  >
    ...
    <span>{TOOL_STATE_LABELS[status]}</span>
  </Badge>
);
...
export const ToolInput = ({ className, input, ...props }: ToolInputProps) => (
  <div
    className={cn(
      "min-w-0 max-w-full space-y-2 overflow-hidden p-4",
      className,
    )}
    {...props}
  >
    ...
    <div className="min-w-0 max-w-full overflow-x-auto rounded-md bg-muted/50">
...
export const ToolOutput = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "min-w-0 max-w-full overflow-x-auto rounded-md text-xs [&_table]:w-auto [&_table]:min-w-full",
          errorText
            ? "bg-destructive/10 text-destructive"
            : "bg-muted/50 text-foreground",
        )}
      >
        {errorText && <div className="min-w-0 max-w-full">{errorText}</div>}
